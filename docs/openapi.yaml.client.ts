...
const UserWithToken = z.object({ user: UserSelf, token: z.string() }).passthrough();
const ErrorResponse = z.object({ message: z.string(), code: z.number().int() }).passthrough();
const LoginRequest = z.object({ email: z.string().email(), password: z.string().min(8) });
const UpdateMe = z.object({ username: z.string(), email: z.string().email(), userAddress: UserAddress }).partial();
...
const UpdateUser = UpdateMe.and(
    z
        .object({ password: z.string().min(8), userRole: UserRole, isActive: z.boolean() })
        .partial()
        .passthrough()
);
...
const GenericItem = ItemBase.and(
    z.object({ kind: z.literal("GenericItem"), frameColor: FrameColor, modulePackage: ModulePackage }).passthrough()
);
const ModuleSize = z.enum(["1x1", "2x1", "3x2", "4x2"]);
const ControllerType = z.enum(["remote", "app"]);
const ModuleItem = ItemBase.and(
    z.object({ kind: z.literal("ModuleItem"), moduleSize: ModuleSize, controllerType: ControllerType }).passthrough()
);
...
const ItemFetch = z.discriminatedUnion("kind", [GenericItem, ModuleItem, AccessoryItem]);
const BaseMaterial = z.enum(["white_polymer", "black_polymer", "wood_oak", "wood_maple", "wood_pine"]);
...
const ItemInput = z.discriminatedUnion("kind", [GenericItemInput, ModuleItemInput, AccessoryItemInput]);
const BaseItemFields = z.object({ itemName: z.string(), description: z.string(), price: z.number() });
...
const postAdminitemsItemIdmedia_Body = z
    .object({ url: z.string().url(), mediaType: MediaType, displayOrder: z.number().int() })
    .passthrough();
const patchAdminitemsItemIdmediaMediaId_Body = z
    .object({ url: z.string().url(), mediaType: MediaType, displayOrder: z.number().int() })
    .partial()
    .passthrough();
const BaseBundleFields = z.object({ bundleName: z.string(), description: z.string(), price: z.number() });
...
const Cart = z
    ...
    .passthrough();
const CartInsert = z.object({ itemId: z.number().int(), quantity: z.number().int().gte(1) }).passthrough();
...
const OrderItemInput = z
    ...
    .passthrough();
const NewOrder = z.object({ shippingAddress: z.string(), items: z.array(OrderItemInput).min(1) }).passthrough();
...
const Review = z
    ...
    .passthrough();
const postReviews_Body = z.object({ rating: z.number().int().gte(1).lte(5), review: z.string() }).passthrough();
